...
const EmailDisplay = ({ email }: Props) => {
    ...
    // if my account address == sender email address lets have a better UI
    const isMe = account?.emailAddress === email.from.address;

  return (
    <div className={
        cn('border rounded-md p-4 transition-all hover:translate-x-2', {
            'border-l-gray-950 border-l-4' : isMe
        })
    }>
        <div className="flex items-center justify-between gap-2">
            <div className="flex items-center justify-between gap-2">
                {!isMe && <Avatar name={email.from.name ?? email.from.address} email={email.from.address} size='35' round textSizeRatio={2} />}
