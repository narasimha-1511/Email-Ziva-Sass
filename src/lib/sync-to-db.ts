...
async function syncEmailsToDatabase(emails : EmailMessage[] , accountID: string){
    ...
    //sending a parallel concurrent requests limiting the no of parallel requests
    ...
    try {
        ...
        console.log('error while updating the emails' , error)
    }
}
...
async function upsertEmail(email: EmailMessage , accountId: string, index: number) {
    ...
    try {
        ...
        if(email.sysLabels.includes('inbox') || email.sysLabels.includes('unread')){
        ...
        if(!fromAddress){
            console.log(`Failed to Upsert from address from the email ${email.bodySnippet}`);
            return;
        }
       ...
       const thread = await db.thread.upsert({
        ...
        for(const threadEmail of threadEmails){
            if(threadEmail.emailLabel === 'draft'){
                threadFolderType = 'draft';
                break;
            }
            else if(threadEmail.emailLabel === 'inbox'){
                threadFolderType = 'inbox';// if any email is in inbox , we know the whole thread is inbox
                break;
            }
        };
        ...
        //last step
        for(const attachement of email.attachments){
            await upsertAttachment(email.id, attachement);
        }
